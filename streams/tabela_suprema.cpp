...
void print_tab(std::vector<std::string>  & rotulos, std::vector<std::string> & value);
...
void read_labels( std::vector<std::string>  & rotulos, std::string arq)
{
    ...
    {
        std::cout << "Erro na leitura do arquivo: " << arq << std::endl;
    }
}
...
void print_tab(std::vector<std::string>  & rotulos, std::vector<std::string> & value)
{
    ...
    for(int i{0}; i < value.size();)
    {
        ...
        {
            std::cout << "|" << std::setw((rotulos[j].size() + 1)) << std::setfill(' ');
            std::cout << value[i] << " ";
            i++;
        }
